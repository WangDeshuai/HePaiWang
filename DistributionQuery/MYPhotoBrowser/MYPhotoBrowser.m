...
//          _rollScrollView.backgroundColor=[UIColor yellowColor];
...
- (instancetype)initWithUrls:(NSArray<NSString *> *)urlString imgViews:(NSArray *)imageViews placeholder:(NSString *)imageName currentIdx:(NSInteger)currentIdx handleNames:(NSArray *)names callback:(longpressCallback)callback
{
    ...
    _callback        = callback;
    CGRect frame = CGRectMake(0,64, kScreen_with, kScreen_height-100);
...
- (void)addphotosScrollView:(NSArray *)handleNames
{
    ...
    for (NSInteger index = 0; index < self.imagesUrlString.count; index ++) {
        ...
        MYZoomScrollView *zooScrollView = [[MYZoomScrollView alloc]initWithFrame:CGRectMake(index *kScreen_with, -20, kScreen_with, kScreen_height)];
...
//        zooScrollView.backgroundColor=[UIColor greenColor];
        //计算imageView大小
