...
- (void)setup
{
    ...
    for (int i = 0; i < 100; i++) {
        ...
        UILongPressGestureRecognizer* gp=[[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(changAn:)];
        gp.view.tag=i;
        [imageView addGestureRecognizer:gp];
...
//    }
        if (_picPathStringsArray.count == 1) {
            ...
            if (image.size.width) {
                itemH = 50;//image.size.height / image.size.width * itemW;
            }
        }
        else {
            itemH = itemW;
...
//
        ...
        [_picPathStringsArray enumerateObjectsUsingBlock:^(NSString *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
           ...
           // NSLog(@"个数啊啊啊%d",rowIndex);
           // NSLog(@"我是>>>%lu",rowIndex);
...
//            [imageView sd_setImageWithURL:[NSURL URLWithString:obj]];
            //  NSLog(@"我是obj>>>>>%@",obj);
            [imageView setImageWithURL:[NSURL URLWithString:obj] placeholderImage:[UIImage imageNamed:@""]];
...
-(void)changAn:(UILongPressGestureRecognizer*)tap{
...
- (CGFloat)itemWidthForPicPathArray:(NSArray *)array
{
    if (array.count == 1) {
        ...
        CGFloat w = [UIScreen mainScreen].bounds.size.width > 320 ? 80 : 60;
        return w;
    }
}
